...
screen nvl(dialogue, items=None):
    ...
    # NVL 下提供存读档与快存快读热键，避免跳过主菜单后无入口。
    key "K_F5" action ShowMenu("save")
    key "K_F6" action QuickSave()
    key "K_F7" action ShowMenu("load")
    ...
    # 部分工程在 NVL 下不会显示 overlay_screens，这里显式挂载快捷菜单。
    if quick_menu:
        use quick_menu
...
style nvl_window:
    ...
    background Transform(Solid("#000000"), alpha=gui.nvl_window_opacity)
...
style nvl_window:
    variant "small"
    background Transform("gui/phone/nvl.png", alpha=gui.nvl_window_opacity)
